(function(){
  ...
  const last=available.find(x=>x.week===state.lastWeek);
  ...
  mount.innerHTML=`
    ...
    <section class="s9-primary-grid">
      <article class="s9-feature"><p class="eyebrow">Continue Your Journey</p><h2>${esc(next.title)}</h2><p>Week ${next.week} · ${esc((next.scripture||[]).join(' • '))}</p><a class="button primary" href="current-events-series.html?week=${next.week}">${complete.size?'Continue Journey':'Start Journey'} →</a></article>
