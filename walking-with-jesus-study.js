(()=>{
  ...
  const routePrefix=spanishRoute?'../':'';
  const englishStudyHref=number=>`${routePrefix}walking-with-jesus-study.html?week=${number}`;
  const spanishStudyHref=number=>`${routePrefix}es/caminando-con-jesus-estudio.html?week=${number}`;
  const collectionHref=targetLang=>targetLang==='es'?`${routePrefix}es/caminando-con-jesus.html`:`${routePrefix}walking-with-jesus.html`;
  const ui=lang==='es'?{
