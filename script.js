...
navigationScript.src='site-navigation.js?v=1.3.0';
...
(function platformIntegration(){
  ...
  const ensureLibrary=()=>new Promise((resolve,reject)=>{if(window.NLDG_LIBRARY){resolve();return;}const script=document.createElement('script');script.src='content-library.js?v=20260728-2';script.onload=resolve;script.onerror=reject;document.head.appendChild(script);});
  ...
  ensureLibrary().then(()=>{
    ...
    const current=window.NLDG_CONTENT.find(item=>item.url===page||item.id===document.body.dataset.studyPage);
...
(function sprintFiveBootstrap(){
  ...
  if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw.js?v=5.5.0').catch(()=>{}));
  let installPrompt;
