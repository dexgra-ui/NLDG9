(function(){
  ...
  function renderLanding(){
    ...
    const nextLesson=series.lessons.find(item=>!completed.has(item.week))||series.lessons[0];
    view.innerHTML=`<div class="series-overview"><div><p class="kicker">About this series</p><h2>See today’s world through the truth of Scripture.</h2><p>This series is designed for small groups, Sunday school, and personal reflection. Each lesson connects a contemporary issue with Scripture, discernment, discussion, prayer, and application.</p><a class="button primary" href="${href(nextLesson.week)}">${completed.size?'Continue the Series':'Begin Week 1'}</a></div><div class="series-progress"><strong>${completed.size} of ${series.lessons.length}</strong><span>Lessons completed</span><progress max="100" value="${progress}">${progress}%</progress><small>${progress}% complete</small></div></div><div class="series-heading"><div><p class="kicker">Lessons</p><h2>All 42 lessons are mapped and ready.</h2></div></div><div class="series-lesson-grid">${series.lessons.map(item=>`<article class="series-lesson-card ${completed.has(item.week)?'is-complete':''}"><div class="week-badge" data-week="${item.week}">${item.week}</div><h3>Week ${item.week} – ${escapeHtml(item.shortTitle||item.title)}</h3><p>${escapeHtml(item.summary)}</p><div class="series-card-meta">${item.unit?`<span>${escapeHtml(item.unit)}</span>`:''}${scripture(item)?`<span>${escapeHtml(scripture(item))}</span>`:''}${completed.has(item.week)?'<span>✓ Complete</span>':''}</div><a href="${href(item.week)}" aria-label="Open Week ${item.week}">Open Lesson</a></article>`).join('')}</div>`;
  }
