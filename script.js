...
const scriptureLinksScript=document.createElement('script');
...
(function platformIntegration(){
  if(!document.querySelector('link[data-platform-styles]')){const link=document.createElement('link');link.rel='stylesheet';link.href='platform.css?v=5.3.2';link.dataset.platformStyles='true';document.head.appendChild(link);}
  ...
  const ensureLibrary=()=>loadScript('content-library.js?v=20260920-11',()=>Boolean(window.NLDG_LIBRARY)).then(()=>loadScript('marriage-family-library.js?v=1.0.0',()=>Boolean(window.NLDG_MARRIAGE_FAMILY_LIBRARY_LOADED))).then(()=>loadScript('difficult-questions-library.js?v=1.0.0',()=>Boolean(window.NLDG_DIFFICULT_QUESTIONS_LIBRARY_LOADED))).then(()=>loadScript('leadership-library.js?v=1.0.0',()=>Boolean(window.NLDG_LEADERSHIP_LIBRARY_LOADED))).then(()=>loadScript('devotional-library.js?v=1.3.0',()=>Boolean(window.NLDG_DEVOTIONAL_LIBRARY_LOADED))).then(()=>loadScript('article-library.js?v=1.0.0',()=>Boolean(window.NLDG_ARTICLE_LIBRARY_LOADED))).then(()=>loadScript('contact-library.js?v=1.0.0',()=>Boolean(window.NLDG_CONTACT_LIBRARY_LOADED)));
  ...
  ensureLibrary().then(()=>{
    ...
    const navigationLandingPages=new Set([
      ...
      'studies.html','standalone-studies.html','book-by-book.html','current-events-series.html','james-series.html',
    ...
    const studyExperiencePages=new Set([
      'studies.html','standalone-studies.html','study-library.html','book-by-book.html','current-events-series.html','james-series.html',
    ...
    const isLessonDetail=Boolean(document.body.dataset.studyPage)||dedicatedLessonPages.has(page)||Boolean(detailParam&&new URLSearchParams(location.search).get(detailParam))||(page==='preparing-walk-with-jesus.html'&&new URLSearchParams(location.search).get('lesson'));
    if(isLessonDetail)document.body.classList.add('compact-lesson-header');
...
(function sprintFiveBootstrap(){
  ...
  if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw.js?v=6.5.3').catch(()=>{}));
  let installPrompt;
  window.addEventListener('beforeinstallprompt',event=>{event.preventDefault();installPrompt=event;const navigation=document.querySelector('.site-header nav');if(!navigation||document.querySelector('.install-app-button'))return;const button=document.createElement('button');button.type='button';button.className='install-app-button';button.textContent='Install App';button.addEventListener('click',async()=>{if(!installPrompt)return;installPrompt.prompt();await installPrompt.userChoice;installPrompt=null;button.remove();});navigation.appendChild(button);});
})();
