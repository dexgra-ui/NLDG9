...
const excludedPages=new Set(['studies.html','study-library.html','current-events-series.html','james-series.html','women-of-faith.html','men-of-faith.html','marriage-family.html','difficult-questions.html','leadership.html','sunday-school.html','technology-ai.html']);
const isIndividual=item=>item.status==='published'&&/study|lesson/i.test(item.type)&&!/collection|library|series/i.test(item.type);
...
for(const item of lessons){
 ...
 const excluded=excludedPages.has(file)&&!item.url.includes('?');
 const eligible=exists&&globalLoader&&contentRoot&&!excluded;
 if(!eligible)failures.push(`${item.id}: ${!exists?'missing page':!globalLoader?'missing script.js':!contentRoot?'missing supported lesson content root':'excluded as a hub page'}`);
