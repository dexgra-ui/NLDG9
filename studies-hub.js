(function(){
  ...
  if(collections){collections.innerHTML=categories.map(category=>{const count=studies.filter(study=>study.category===category).length;const detail=categoryDetails[category]||{icon:'📘',description:'Explore studies in this collection.'};return `<button data-collection="${escapeHtml(category)}"><span>${detail.icon}</span><h3>${escapeHtml(category)}</h3><p>${escapeHtml(detail.description)}</p><small>${count} ${count===1?'lesson':'lessons'}</small></button>`;}).join('')+`<button data-collection="all"><span>✨</span><h3>Complete Library</h3><p>Browse every published study and lesson.</p><small>${studies.length} lessons</small></button>`;}
  const renderDashboard=()=>{const state=readState();const completed=studies.filter(study=>state[study.id]?.completed).length;const inProgress=studies.filter(study=>!state[study.id]?.completed&&(state[study.id]?.progress||0)>0).length;const favorites=studies.filter(study=>state[study.id]?.favorite);if(dashboard)dashboard.innerHTML=`<div class="dashboard-stat"><strong>${completed}</strong><span>Completed</span></div><div class="dashboard-stat"><strong>${inProgress}</strong><span>In Progress</span></div><div class="dashboard-stat"><strong>${favorites.length}</strong><span>Favorites</span></div>`;if(favoritesSection&&favoritesGrid){favoritesSection.hidden=favorites.length===0;favoritesGrid.innerHTML=favorites.map(card).join('');}};
  if(grid)grid.innerHTML=studies.map(card).join('');
  renderDashboard();
  const featuredStudy=studies.find(study=>study.featured)||studies[0];
  if(featured&&featuredStudy){featured.innerHTML=`<div><span class="content-type">Featured Study</span><h2>${escapeHtml(featuredStudy.title)}</h2><p>${escapeHtml(featuredStudy.description)}</p><div class="study-meta"><span>📖 ${escapeHtml((featuredStudy.scripture||[]).join(', '))}</span><span>⏱ ${escapeHtml(featuredStudy.duration)} minutes</span><span>${escapeHtml(featuredStudy.difficulty||'All Levels')}</span></div><div class="actions"><a class="button primary study-open" data-study-id="${escapeHtml(featuredStudy.id)}" href="${escapeHtml(featuredStudy.url)}">Begin Study</a><a class="button secondary" href="scripture-index.html">Browse by Scripture</a></div></div><div class="featured-study-symbol">📖</div>`;}
  document.addEventListener('click',event=>{const button=event.target.closest('[data-favorite-id]');if(!button)return;const id=button.dataset.favoriteId;const state=readState();state[id]={...(state[id]||{}),favorite:!state[id]?.favorite,updated:Date.now()};writeState(state);document.querySelectorAll(`[data-favorite-id="${CSS.escape(id)}"]`).forEach(item=>{item.setAttribute('aria-pressed',String(state[id].favorite));item.textContent=state[id].favorite?'★':'☆';});renderDashboard();});
})();