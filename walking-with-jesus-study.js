(()=>{
  ...
  const lang=spanishRoute||p.get('lang')==='es'?'es':'en';
  ...
  const languageSwitch=number=>`<div class="wj-language-switch" aria-label="Language / Idioma"><a class="${lang==='en'?'is-active':''}" href="${englishStudyHref(number)}">English</a><a class="${lang==='es'?'is-active':''}" href="${spanishStudyHref(number)}">Español</a></div>`;
  if(!lesson){root.innerHTML=`<section class="wj-hero"><h1>${lang==='es'?'Lección no encontrada':'Lesson not found'}</h1><a class="button primary" href="${collectionHref(lang)}">${lang==='es'?'Volver a la colección':'Return to the collection'}</a></section>`;return;}
  ...
  const href=n=>lang==='es'?spanishStudyHref(n):englishStudyHref(n);
