...
const enData='romans-study-data.js',enGuide='romans-study-guide.js',esData='romans-study-data-es.js',enPage='romans-study.html',esPage='es/romanos-estudio.html',hub='es/estudios-biblicos.html',i18n='nldg-i18n.js';
...
if(!errors.length){
 ...
 if(es.slug!=='romanos-estudio')fail('Spanish Romans slug must be romanos-estudio.');
 if(es.book!=='Romanos')fail('Spanish book name must be Romanos.');
 if(es.scriptureStandard!=='Nueva Traducción Viviente (NTV)')fail('Spanish Romans must declare Nueva Traducción Viviente (NTV).');
 if(es.themeLabel!=='Verdad clave')fail('Spanish Romans theme label must be Verdad clave.');
 if(es.lessonSubtitleMode!==true||en.lessonSubtitleMode!==true)fail('Romans must retain lesson subtitle mode.');
 if(en.lessons?.length!==8||es.lessons?.length!==8)fail('Romans must retain eight lessons in both languages.');
 ...
 for(const f of ['seriesJesusConnection','seriesGuardrail','seriesClosingTakeaway','seriesExamination','seriesPractice','seriesLeaderGuidance','seriesPrayer'])if(!String(es[f]||'').trim())fail('Spanish Romans series missing '+f+'.');
 for(let i=0;i<8;i++){
  const a=en.lessons[i],b=es.lessons[i],label='Romans lesson '+(i+1);
 ...
 for(const phrase of ['acoso','antisemit','pueblo judío','torá','esclavitud','romanOS 8:28'.toLowerCase(),'autoridad','obediencia ilimitada','conciencia','mujeres','abuso','nueva traducción viviente'])if(!all.includes(phrase.toLowerCase()))fail('Missing safeguard/theme: '+phrase);
 for(const version of ['RVR60','NVI','NBLA'])if(new RegExp('\\b'+version+'\\b').test(JSON.stringify(es)))fail('Spanish Romans contains disallowed Bible version '+version+'.');
 const ep=read(enPage),sp=read(esPage),im=read(i18n),hb=read(hub);
 if(!ep.includes('hreflang="es" href="https://nolabelsdesignedbygod.org/es/romanos-estudio.html"'))fail('English bilingual route missing.');
 if(!sp.includes('hreflang="en" href="https://nolabelsdesignedbygod.org/romans-study.html"'))fail('Spanish bilingual route missing.');
 if(!sp.includes('romans-study-data-es.js?v=1.1.0')||!sp.includes('book-study-series.js?v=0.2.0')||!sp.includes('book-study-series-es.js?v=1.2.0'))fail('Spanish Romans assets are stale.');
 if(!im.includes("'romans-study.html':'es/romanos-estudio.html'"))fail('i18n Romans route missing.');
...
console.log('Spanish Romans audit passed: English/Spanish reference parity, NTV standard, 5/8/8/2 structure, safeguards, assets, routes, and completed library state validated.');
