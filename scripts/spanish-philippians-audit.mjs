...
const enData='philippians-study-data'+js,enGuide='philippians-study-guide'+js,esData='philippians-study-data-es'+js,enPage='philippians-study'+html,esPage=['es','filipenses-estudio'+html].join('/'),hub=['es','estudios-biblicos'+html].join('/'),i18n='nldg-i18n'+js;
...
if(!errors.length){
 ...
 if(es.slug!=='filipenses-estudio')fail('Spanish Philippians slug must be filipenses-estudio.');
 if(es.book!=='Filipenses')fail('Spanish book name must be Filipenses.');
 if(es.scriptureStandard!=='Nueva Traducción Viviente (NTV)')fail('Spanish Philippians must declare Nueva Traducción Viviente (NTV).');
 if(es.themeLabel!=='Verdad clave')fail('Spanish Philippians theme label must be Verdad clave.');
 if(es.lessonSubtitleMode!==true||en.lessonSubtitleMode!==true)fail('Philippians must retain lesson subtitle mode.');
 if(en.lessons?.length!==6||es.lessons?.length!==6)fail('Philippians must retain six lessons in both languages.');
 ...
 for(const f of ['seriesJesusConnection','seriesGuardrail','seriesClosingTakeaway','seriesExamination','seriesPractice','seriesLeaderGuidance','seriesPrayer'])if(!String(es[f]||'').trim())fail('Spanish Philippians series missing '+f+'.');
 for(let i=0;i<6;i++){
  const a=en.lessons[i],b=es.lessons[i],label='Philippians lesson '+(i+1);
 ...
 for(const phrase of ['gozo','autolesión','abuso','antisemitismo','pueblo judío','ansiedad','salud mental','filipenses 4:13','filipenses 4:19','prosperidad','nueva traducción viviente'])if(!all.includes(phrase.toLowerCase()))fail('Missing safeguard/theme: '+phrase);
 for(const version of ['RVR60','NVI','NBLA'])if(new RegExp('\\b'+version+'\\b').test(JSON.stringify(es)))fail('Spanish Philippians contains disallowed Bible version '+version+'.');
 const ep=read(enPage),sp=read(esPage),im=read(i18n),hb=read(hub);
 if(!ep.includes('hreflang="es" href="https://nolabelsdesignedbygod.org/es/filipenses-estudio'+html+'"'))fail('English bilingual route missing.');
 if(!sp.includes('hreflang="en" href="https://nolabelsdesignedbygod.org/philippians-study'+html+'"'))fail('Spanish bilingual route missing.');
 if(!sp.includes('philippians-study-data-es'+js+'?v=1.1.0')||!sp.includes('book-study-series'+js+'?v=0.2.0')||!sp.includes('book-study-series-es'+js+'?v=1.2.0'))fail('Spanish Philippians assets are stale.');
 if(!im.includes("'philippians-study"+html+"':'es/filipenses-estudio"+html+"'"))fail('i18n Philippians route missing.');
 if(!hb.includes('href="filipenses-estudio'+html+'"'))fail('Spanish library route missing.');
 if(!hb.includes('Sesenta y seis series completas y revisadas'))fail('Spanish library completion state missing.');
}
if(errors.length){console.error('Spanish Philippians audit failed:\n- '+errors.join('\n- '));process.exit(1)}
console.log('Spanish Philippians audit passed: English/Spanish reference parity, NTV standard, six-lesson 5/8/8/2 structure, safeguards, assets, routes, and completed library state validated.');
