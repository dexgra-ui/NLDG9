...
const mount=()=>{const header=document.querySelector('.site-header,.spanish-header');if(!header||header.querySelector('.nldg-language-switcher'))return;const switcher=document.createElement('div');switcher.className='nldg-language-switcher';switcher.setAttribute('aria-label',isSpanish?'Selector de idioma':'Language selector');switcher.innerHTML=`<a href="${englishHref}" lang="en"${!isSpanish?' aria-current="true"':''}>English</a><span class="separator" aria-hidden="true">|</span><a href="${spanishHref}" lang="es"${isSpanish?' aria-current="true"':''}>Español</a>`;const menu=header.querySelector('.menu');header.insertBefore(switcher,menu||header.querySelector('nav')||null)};
