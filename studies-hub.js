(function(){
  ...
  const card=study=>{
    ...
    return `<article class="study-card" data-study-id="${escapeHtml(study.id)}" data-category="${escapeHtml(study.category)}" data-search="${escapeHtml([study.title,study.description,scripture,tags].join(' ').toLowerCase())}"><div class="study-topline"><span>${escapeHtml(study.category)}</span><small>${escapeHtml(study.difficulty||'All Levels')}</small></div><h3>${escapeHtml(study.title)}</h3><p>${escapeHtml(study.description)}</p><div class="study-meta"><span>📖 ${escapeHtml(scripture||'Scripture study')}</span>${study.series==='Walking with Jesus'?'<span>Use at your own pace</span>':`<span>⏱ ${escapeHtml(study.duration||45)} minutes</span>`}</div>${progress?`<div class="study-card-progress"><span>${state.completed?'Completed':`${progress}% complete`}</span><progress max="100" value="${progress}">${progress}%</progress></div>`:''}<div class="study-experience-actions"><a class="study-open" data-study-id="${escapeHtml(study.id)}" href="${escapeHtml(study.url)}">${progress&&!state.completed?'Continue':'Open Lesson'} →</a><button class="favorite-study" type="button" data-favorite-id="${escapeHtml(study.id)}" aria-label="${state.favorite?'Remove from favorites':'Add to favorites'}" aria-pressed="${state.favorite?'true':'false'}">${state.favorite?'★':'☆'}</button></div></article>`;
  };
